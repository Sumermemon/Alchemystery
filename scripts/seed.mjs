...
async function seed() {
  ...
  if (catErr) {
    console.error('Error category:', catErr);
    return;
  }
  ...
  for (const s of services) {
    ...
    if (error) console.error('Error service:', s.slug, error);
  }
  ...
  if (tErr) console.error('Error testimonial:', tErr);
  ...
  for (const p of posts) {
    ...
    if (error) console.error('Error post:', p.slug, error);
  }
