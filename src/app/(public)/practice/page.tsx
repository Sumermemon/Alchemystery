...
export async function generateMetadata(): Promise<Metadata> {
  const [page, settings] = await Promise.all([
    getPublishedPageBySlug('practice'),
    getSiteSettings()
  ]);
  
  if (!page) {
    return { title: 'The Practice' };
  }
...
export default async function PracticePage() {
  const page = await getPublishedPageBySlug('practice');
  ...
  return (
    <>
      <div className="text-center pt-24 pb-12">
        <p className="text-[var(--color-gold)] text-xs tracking-[0.2em] uppercase mb-4">The Practice</p>
